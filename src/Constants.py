...
TMP_MODELS_PATH = os.path.join(DATA_PATH, "tmp_models")
...
makedir_if_not_exists(TMP_MODELS_PATH)
...
def neighbour_sum_radius_name(idx, atom=None):
    if atom is None:
        ...
        return (
            "neighbour_sum_radius_" + str(atom) + "_" + str(NEIGHBOUR_SUM_RADIUS[idx])
        )
...
def neighbour_sum_above_plane_radius_name(idx, atom=None):
    if atom is None:
        ...
        return (
            "neighbour_sum_radius_"
            + str(atom)
            + "above_plane"
            + str(NEIGHBOUR_SUM_RADIUS[idx])
        )
...
NODE_FEATURES_NUM = (
    5 + len(NODE_APPENDED_FEATURES) + len(NODE_GROUP_FEATURES) + DSSP_FEATURES_NUM
)
...
FEATURE_NAMES = ["mass", "occupancy", "element", "fullname", "aa"]
